...
int process_response(struct http_request *request, struct http_response *response){
	...
	if(!strcmp(response->status_code,RESOURCE_NOT_FOUND)){
		...
		if(delete_resource(request->path)||delete_meta_resource(request->path)){
			return FAILURE;
		}
		return SUCCESS;
	}
	...
	if(!strcmp(response->status_code,PARTS_FOUND)){
		char old_cache[BUFFER_SIZE], new_cache[BUFFER_SIZE];
		if(get_resource(request->path,old_cache)==FAILURE){
			return FAILURE;
		}
...
int tokenize_delta(char *delta, struct delta_list **delta_head){
	...
	while(*delta_pointer){
		...
		while(*delta_pointer!=RANGE_START){
			if(!*delta_pointer){
				return SUCCESS;
			}
			delta_pointer++;
		...
		char *byte_start=strtok_r(delta_pointer, RANGE_SEPARATOR, &delta_pointer);
		char *byte_end=strtok_r(delta_pointer, RANGE_END, &delta_pointer);

		current_node->byte_start=atol(byte_start);
		current_node->byte_end=atol(byte_end);

		//strtok_r(delta_pointer, SIZE_START, &delta_pointer);
		while(*delta_pointer!=SIZE_START){
			if(!*delta_pointer){
				return FAILURE;
			}
			delta_pointer++;
